...
JNIEXPORT jint JNICALL Java_org_araqne_winapi_Process_GetProcessMemoryInfo
...
JNIEXPORT jint JNICALL Java_org_araqne_winapi_Process_GetProcessorCount
...
JNIEXPORT jint JNICALL Java_org_araqne_winapi_Process_GetProcessTimes
(JNIEnv *env, jobject obj, jint hProcess, jobject creationTime, jobject exitTime, jobject kernelTime, jobject userTime)
{
	...
	ret = GetProcessTimes((HANDLE)hProcess, &creation, &exit, &kernel, &user);
...
JNIEXPORT jint JNICALL Java_org_araqne_winapi_Process_EnumProcesses
...
JNIEXPORT jint JNICALL Java_org_araqne_winapi_Process_OpenProcess
(JNIEnv *env, jobject obj, jint desiredAccess, jint inheritHandle, jint processId)
{
	...
	return (jint)OpenProcess((DWORD)desiredAccess, (BOOL)inheritHandle, (DWORD)processId);
}

JNIEXPORT jstring JNICALL Java_org_araqne_winapi_Process_GetModuleBaseName
(JNIEnv *env, jobject obj, jint hProcess, jint maxLength)
{
	...
	if(EnumProcessModules((HANDLE)hProcess, &hMod, sizeof(hMod), &cbNeeded)) {
		DWORD length = GetModuleBaseName((HANDLE)hProcess, hMod, szProcessName, maxLength);
		if(length > 0)
...
JNIEXPORT jint JNICALL Java_org_araqne_winapi_Process_CloseHandle
